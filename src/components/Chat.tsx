...
import { socket, URL } from "../utils/socket";
...
export const Chat = ({ styles, roomId, beforeSentMessage }: IChat) => {
    ...
    <div className="vTrx6SideChat_container" style={styles?.mainBlock}>
      <div className="vTrx6SideChat_scrollView" style={styles?.scrollBlock}>
        {chat.map((item) => {
          return (
            <div
              key={item._id}
              className="vTrx6SideChat_messageItem"
              style={styles?.scrollBlock_messageBlock}
            >
              <div
                className="vTrx6SideChat_avatarContainer"
              ...
              >
                <img
                  ...
                  alt="avatar"
                  title="avatar"
                  className="vTrx6SideChat_avatar"
                />
              </div>

              <div className="vTrx6SideChat_messageBlock">
                <p
                  className="vTrx6SideChat_title"
                  style={styles?.scrollBlock_messageBlock_title}
                >
                  {item?.user?.userName}
                </p>
                <p
                  className="vTrx6SideChat_textContent"
                  style={styles?.scrollBlock_messageBlock_message}
                >
                  {item?.message}
                  <span className="vTrx6SideChat_messageDate">
        ...
        {isLoading && (
          <div className="vTrx6SideChat_loaderContainer">
            <div className="vTrx6SideChat_loader" />
          </div>
        )}

        {!isLoading && !error && chat?.length === 0 && (
          <p className="vTrx6SideChat_dummy">Chat history is empty...</p>
        )}
        {error && <p className="vTrx6SideChat_err">{error}</p>}
      ...
      >
        <input
          ...
          type="text"
          className="vTrx6SideChat_input"
          placeholder="Message..."
          style={styles?.inputBlock_input}
        />
        <button
          type="submit"
          className="vTrx6SideChat_submit"
